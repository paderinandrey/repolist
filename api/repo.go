...
// week represents the summary of a week of activity on a repository
type week struct {
	ID           int
	RepositoryID int
	WeekNumber   int
...
// timeline represents important events happened on a repository,
// which will be displayed on the repo timeline
...
// parseParams accept a req and returns the `num` path tokens found after the `prefix`.
// returns an error if the number of the token are less or more than expected
